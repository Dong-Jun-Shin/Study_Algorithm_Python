# 원형 이중 연결리스트 클래스 DoubleLinkedList 사용하기

from enum import Enum
from double_list import DoubleLinkedList

Menu = Enum('Menu', ['머리에노드삽입', '꼬리에노드삽입', '주목노드바로뒤삽입',
                        '머리노드삭제', '꼬리노드삭제', '주목노드출력', '주목노드이동', '주목노드역순이동',
                        '주목노드삭제', '모든노드삭제', '검색', '멤버십판단', '모든노드출력',
...
while True:
    ...
    elif menu == Menu.주목노드바로뒤삽입:         # 주목 노드 바로 뒤에 노드를 삽입
        lst.add(int(input('주목 노드 바로 뒤에 넣을 값을 입력하세요.: ')))

    elif menu == Menu.머리노드삭제:              # 맨 앞에 노드를 삭제
        lst.remove_first()

    elif menu == Menu.꼬리노드삭제:              # 맨 끝애 노드를 삭제
        lst.remove_last()

    elif menu == Menu.주목노드출력:              # 주목 노드를 출력
        lst.print_current_node()

    elif menu == Menu.주목노드이동:              # 주목 노드를 뒤로 이동
        lst.next()
    ...
    elif menu == Menu.멤버십판단:                # 특정 노드가 있는지 판단
